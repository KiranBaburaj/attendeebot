...
class WebBotAdapter(BotAdapter):
    ...
    def init(self):
        ...
        if platform.system() != "Windows" and os.environ.get("DISPLAY") is None:
            # Create virtual display only if not on Windows and no real display is available
            self.display = Display(visible=0, size=(1930, 1090))
            self.display.start()
            self.display_var_for_debug_recording = self.display.new_display_var
        elif platform.system() == "Windows":
            # On Windows, set a dummy display variable
            self.display = None
            self.display_var_for_debug_recording = ":0"

        if self.should_create_debug_recording:
            # Only start debug recording if not on Windows or if display is available
            if platform.system() != "Windows" or self.display is not None:
