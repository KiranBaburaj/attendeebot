...
try:
    ...
    gi = None
    print("Warning: PyGObject (gi) not available")
import redis
...
class BotController:
    ...
    def get_recording_file_location(self):
        if self.pipeline_configuration.rtmp_stream_audio or self.pipeline_configuration.rtmp_stream_video:
            ...
            # Create a tmp directory in the current working directory if it doesn't exist
            tmp_dir = os.path.join(os.getcwd(), 'tmp')
            if not os.path.exists(tmp_dir):
                os.makedirs(tmp_dir)
            return os.path.join(tmp_dir, self.get_recording_filename())
    ...
    def connect_to_redis(self):
        ...
        # Fix: Don't append SSL parameters to URL, use the plain Redis URL
    ...
    def run(self):
        ...
        try:
            ...
            # Only add unix signal handlers on Unix-like systems
            import platform
